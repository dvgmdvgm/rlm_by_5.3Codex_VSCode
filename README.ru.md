...
- Локальный синтез по файлам проекта: `local_workspace_brief(question: str, project_path: str | None = None, ...)`
...
Маршрутизация retrieval:

- `local_memory_bootstrap(...)` теперь возвращает `retrieval_strategy`.
- Для UI/template/layout-задач bootstrap может дополнительно вернуть `workspace_brief` и `workspace_selected_files`, локально сжав релевантные файлы проекта перед облачным этапом.
- Цель: уменьшить cloud-токены на больших шаблонах и стилях за счёт локального narrowing + summarization.
