...
3. Call `get_memory_metadata(project_path=<active_workspace_root>, max_files=20, include_headers=false)`.
4. Read canonical memory first (when present):
   - `memory/canonical/architecture.md`
   - `memory/canonical/coding_rules.md`
   - `memory/canonical/active_tasks.md`
5. Use memory findings to shape implementation choices and avoid regressions.

### B) During implementation

- Follow decisions from canonical memory unless user explicitly overrides.
- If memory is stale or missing key info, extract facts via REPL (`execute_repl_code(..., project_path=...)` + `llm_query` / `llm_query_many`) from relevant memory files only.
