...
- `consolidate_memory(log_rel_path="logs/extracted_facts.jsonl", write_changelog=True, refresh_context=True, project_path=None, summarize_old_changelogs=True, older_than_days=2, keep_raw_changelogs=False, max_files_per_summary=20, max_changelog_files_trigger=40, max_changelog_bytes_trigger=25000)`
  - compacts extracted facts into canonical memory and changelog

Cloud payload log behavior:
- `memory/logs/cloud_payload_audit.md` is append-only audit history with compact previews (`payload_preview`).
- `memory/logs/cloud_payload_current.md` is a single overwrite snapshot of the latest cloud-facing payload with full content (`payload_full`).
- both files are generated by deterministic local Python code (no LLM formatting path).
...
- re-evaluate all active rules independently for each approved task (no carry-over match/skip state),
- execute `action` only for matched rules (scope + trigger + preconditions),
- return mandatory diagnostics:
  - `RULES_CHECKED`
  - `RULES_MATCHED`
  - `RULES_EXECUTED`
  - `RULES_FAILED_NONBLOCKING`
  - `RULES_FAILED_BLOCKING`
  - `RULES_EVIDENCE_COMPLETE`
  - `RULE_EXECUTION_SUMMARY`.

Strict `OP_RULES_OK` pass criteria:
- every matched rule has an execution attempt record,
- every matched rule includes evidence fields: `command`, `exit_code`, `output_summary`,
...
Structured-first persistence guidance:
- persist operational rule payload as JSON string in `value.value` (strict extracted_fact schema),
- include `rule_id` both in `value.entity` and payload JSON,
- verify exact `RULE_ID` token appears in canonical files after consolidation.
