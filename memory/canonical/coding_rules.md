...
### local_workspace_brief_tool
- [feature][active;p=8] Added local_workspace_brief MCP tool that scans narrowed workspace source files, summarizes them with the local LLM, and returns compact file-level context for large UI/template/layout tasks to reduce cloud token usage. (source: session:workspace_routing_20260307)
...
### workspace_retrieval_strategy
- [feature][active;p=8] local_memory_bootstrap now adds retrieval_strategy hints that classify tasks as symbol_lookup, ui_template, bugfix, refactor, or general_code. UI/template tasks prefer local_workspace_brief, while symbol/code tasks prefer code index tools first when available. (source: session:workspace_routing_20260307)
